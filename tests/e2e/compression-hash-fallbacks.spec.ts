...
import { disableGlobal } from '../__helpers__/disable-global.js';
...
for (const featureName of FEATURES_TO_DISABLE) {
  test.describe(`Hash consistency (fallback mode: ${featureName} disabled)`, () => {
    test.beforeEach(async ({ page }) => {
      const disableScript = disableGlobal(featureName);

      await page.addInitScript(disableScript);
    });
    ...
    for (const testImage of TEST_IMAGES) {
      test(`should produce consistent hash for ${testImage.name}`, async ({
        ...
        const downloadPath = path.join(
          __dirname,
          `../__tmp__/${browserName}/${testImage.name}.${featureName}.fallback.processed.jpeg`
        );
